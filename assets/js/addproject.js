...
function addProject(event) {
  ...
  let durationYear = Math.floor(
    durationDate /
      (miliSecond * secondInHour * hourInDay * dayInHour * monthInYear)
  );
  let durationMonth =
    Math.floor(
      durationDate / (miliSecond * secondInHour * hourInDay * dayInHour)
    ) % monthInYear;
  let durationDay =
    Math.floor(durationDate / (miliSecond * secondInHour * hourInDay)) %
    dayInHour;
  let duration = "";
  ...
  //Checked box
  const checkSwift = '<img src="/assets/image/swift.png">';
  const checkRuby = '<img src="/assets/image/ruby.png">';
  const checkPhyton = '<img src="/assets/image/phython.png">';
  const checkJavascript = '<img src="/assets/image/js.png">';
...
function renderProject() {
  ...
  for (let i = 0; i < blog.length; i++) {
    document.getElementById("new-page").innerHTML += `
        <div class="card">
            <a href="/project=${i}" target="_blank">
                <img src="${blog[i].image}" alt="">
            </a>
            <h1>${blog[i].project}</h1>
            <p style="color: gray">Durasi : ${blog[i].duration}</p>
            ...
            <p style="float: right; color: gray;">${getDistanceTime(
              blog[i].date
            )}</p>
